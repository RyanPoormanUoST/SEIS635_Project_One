...
## Rules
1, Played in a series of rounds
2, The game starts by selecting the game settings. These settings include:
- specifying the number of users
- picking the cards rulesets
- setting the speed of each round 
3. Once the game has started, each round randomly displays a new card for a set period of time. Once that card disappears, each player must enter the card shown in the order of its corresponding round.

For Example: Round #1 shows a 5 of spades. Each user would enter the corresponding card. Round #2 shows a King of hearts, each player would then have to enter a 5 of spades and then a King of hearts.

4. If a player enters a wrong card or a card out of sequence, they are out of the game.
